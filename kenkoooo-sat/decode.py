...
for i in range(1, variable_count + 1):
    if i not in assignments:
        print(f"missing assignment for variable {i}")
        v.append(1)
    else:
...
def submit(x):
    print(f"submit {x}")
    subprocess.call(
        f"python3 ../cli.py send -s -d 'solve efficiency{problem_id} {x}'", shell=True
    )
...
# submit_around(to_decimal(v))
# submit_around(to_decimal(reversed(v)))
